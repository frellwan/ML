...
mypredict = function(){
        ...
        # lambda for boxcox transformation (Using BoxCox from forecast package)
        lam = 0.5        
        ...
        for (i in 1:nrow(unique_pairs)) {
                ...
                # Negative Sales? What, are we paying people to buy stuff?
                # Set any negative sales to 0 
                tmp_train[tmp_train[,'Weekly_Sales'] < 0, 'Weekly_Sales'] = 0                
                
                mycoef <- lm.fit(as.matrix(tmp_train[, -(2:4)]), BoxCox(tmp_train$Weekly_Sales, lambda=lam))$coefficients
                mycoef[is.na(mycoef)] <- 0
                tmp_pred <- InvBoxCox(mycoef[1] + as.matrix(tmp_test[, 4:55]) %*% mycoef[-1], lambda=lam)
